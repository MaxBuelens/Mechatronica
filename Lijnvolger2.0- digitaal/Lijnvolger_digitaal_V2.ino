...
void loop() {
  ...
  if (Active) {
    ...
    if ((SensorRRValue == HIGH && SensorRValue == HIGH && SensorLLValue == LOW) || (SensorRRValue == HIGH && SensorMValue == LOW && SensorLLValue == LOW)) {                            //Middelste, rechtse en uiterste rechtse sensor wit, andere zwart
      // Doe dit tot robot weer op recht op lijn zit anders blijven draaien
...
void TurnLeft() {
  digitalWrite(Enable, HIGH);
  analogWrite(MotorLeftForward, 70);
  analogWrite(MotorLeftBackward, 0);
  analogWrite(MotorRightForward, 120);
  analogWrite(MotorRightBackward, 0);
  Serial.println("Left");
...
void TurnRight() {
  ...
  analogWrite(MotorLeftForward, 120);
  analogWrite(MotorLeftBackward, 0);
  analogWrite(MotorRightForward, 70);
  analogWrite(MotorRightBackward, 0);
