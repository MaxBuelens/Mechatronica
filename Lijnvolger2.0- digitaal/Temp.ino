...
void loop() {
  ...
  if (Active = true) {
    if ((SensorLLValue == LOW && SensorLValue == HIGH && SensorMValue == HIGH && SensorRValue == HIGH && SensorRRValue == LOW) ||
        (SensorLLValue == HIGH && SensorLValue == HIGH && SensorMValue == HIGH && SensorRValue == HIGH && SensorRRValue == HIGH)) { //Middelste sensor wit, andere zwart
      Function vSnelheid("HIGH", vNormaal, 0, vNormaal, 0, "Forward");  //Voorwaarts
    }//End if forward
      ...
      Function vSnelheid("HIGH", vDraaien, 0, vNormaal, 0, "Right"); //Naar rechts
      ...
      Function vSnelheid("HIGH", vNormaal, 0, vDraaien, 0, "Left"); //Naar links
      ...
      while (!(SensorRValue == HIGH && SensorRRValue == LOW ) && Active && !(SensorLLValue == HIGH && SensorLValue == HIGH && SensorMValue == HIGH && SensorRValue == HIGH && SensorRRValue == HIGH)) {
        Function vSnelheid("HIGH", v90Graden, 0, 0, 0, "SharpTurnRight"); //Scherp naar rechts
        ReadSensorAndButton();
      }// End While
      //Stop();
      ...
      while (!(SensorLValue == HIGH && SensorLLValue == LOW) && Active && !(SensorLLValue == HIGH && SensorLValue == HIGH && SensorMValue == HIGH && SensorRValue == HIGH && SensorRRValue == HIGH)) {
        Function vSnelheid("HIGH", 0, 0, 0, v90Graden, "SharpTurnLeft"); //Scherp naar links
        ReadSensorAndButton();
      }// End While
      //Stop();
    }//End else if SharpTurnRight
...
void vSnelheid(String EnableWaarde, int LeftFront, int LeftBack, int RightFront, int RightBack, String strRichting) {
  digitalWrite(Enable, EnableWaarde);
  analogWrite(MotorLeftForward, LeftFront);
  analogWrite(MotorLeftBackward, LeftBack);
  analogWrite(MotorRightForward, RightFront);
  analogWrite(MotorRightBackward, RightBack);
