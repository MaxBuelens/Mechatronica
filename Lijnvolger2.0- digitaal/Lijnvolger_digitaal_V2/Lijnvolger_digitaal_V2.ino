...
void loop() {
  ...
  if (Active) {
    ...
    if ((SensorRRValue == HIGH && SensorRValue == HIGH && SensorLLValue == LOW) || (SensorRRValue == HIGH && SensorMValue == LOW && SensorLLValue == LOW))s {                            //Middelste, rechtse en uiterste rechtse sensor wit, andere zwart
      // Doe dit tot robot weer op recht op lijn zit anders blijven draaien
...
void SharpTurnRight() {
  digitalWrite(Enable, HIGH);
  analogWrite(MotorLeftForward, 80);//100
  analogWrite(MotorLeftBackward, 0);
